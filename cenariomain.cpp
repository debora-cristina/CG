...
void MoveCamera(int direcao) {
	...
	if (xcamera>990.0f) {
		if (direcao*(dxcamera)*0.7 < 0)
			xcamera = xcamera + direcao*(dxcamera)*0.7f;
	}
	if (xcamera<-990.0f) {
		if (direcao*(dxcamera)*0.7 > 0)
			xcamera = xcamera + direcao*(dxcamera)*0.7f;
	...
	if (zcamera>990.0f) {
		if (direcao*(dzcamera)*0.7f < 0)
			zcamera = zcamera + direcao*(dzcamera)*0.7f;
	}
	if (zcamera<-990.0f) {
		if (direcao*(dzcamera)*0.7f > 0)
			zcamera = zcamera + direcao*(dzcamera)*0.7f;
...
void lerArquivo(char* nomeArquivo)
{
	...
	for (i = 1; i < linhas + 1; i++) {
		...
		if (i > m)
		{
			...
			{
				glTexCoord2f(( count1 + poligono.getPontos()[(unsigned int)value[i][j]].getX()) / 30, ( count1 + poligono.getPontos()[(unsigned int)value[i][j]].getZ() / 30));
				glVertex3f(poligono.getPontos()[(unsigned int)value[i][j]].getX(), poligono.getPontos()[(unsigned int)value[i][j]].getY(), poligono.getPontos()[(unsigned int)value[i][j]].getZ());
...
void lerCasa2_telha(char* nomeArquivo)
{
	...
	for (i = 1; i < linhas + 1; i++) {
		...
		if (i > m)
		{
			...
			for (int j = 1; j <= value[i][0]; j++)
			{
				glTexCoord2f((poligono.getPontos()[(unsigned int)value[i][j]].getX()) / 1, (poligono.getPontos()[(unsigned int) value[i][j]].getZ() / 1));
				glVertex3f(poligono.getPontos()[(unsigned int)value[i][j]].getX(), poligono.getPontos()[(unsigned int)value[i][j]].getY(), poligono.getPontos()[(unsigned int) value[i][j]].getZ());
...
void lerCasa2_faixa(char* nomeArquivo)
{
	...
	for (i = 1; i < linhas + 1; i++) {
		...
		if (i > m)
		{
			...
			for (int j = 1; j <= value[i][0]; j++)
			{
				glTexCoord2f((poligono.getPontos()[(unsigned int)value[i][j]].getX()) / 90, (poligono.getPontos()[(unsigned int)value[i][j]].getZ() / 90));
				glVertex3f(poligono.getPontos()[(unsigned int)value[i][j]].getX(), poligono.getPontos()[(unsigned int)value[i][j]].getY(), poligono.getPontos()[(unsigned int)value[i][j]].getZ());
...
void lerCasa1_telha(char* nomeArquivo)
{
	...
	for (i = 1; i < linhas + 1; i++) {
		...
		if (i > m)
		{
			...
			for (int j = 1; j <= value[i][0]; j++)
			{
				glTexCoord2f((poligono.getPontos()[(unsigned int)value[i][j]].getX()) / 30, (poligono.getPontos()[(unsigned int)value[i][j]].getZ() / 30));
				glVertex3f(poligono.getPontos()[(unsigned int) value[i][j]].getX(), poligono.getPontos()[(unsigned int) value[i][j]].getY(), poligono.getPontos()[(unsigned int)value[i][j]].getZ());
...
void lerCasa1_madeira2(char* nomeArquivo)
{
	...
	for (i = 1; i < linhas + 1; i++) {
		...
		if (i > m)
		{
			...
			{
				glTexCoord2f((poligono.getPontos()[(unsigned int)value[i][j]].getX()) / 90, (poligono.getPontos()[(unsigned int)value[i][j]].getZ() / 90));
				glVertex3f(poligono.getPontos()[(unsigned int) value[i][j]].getX(), poligono.getPontos()[(unsigned int)value[i][j]].getY(),
					poligono.getPontos()[(unsigned int)value[i][j]].getZ());
...
void lerArquivoBridge(char* nomeArquivo)
{
	...
	for (i = 1; i < linhas + 1; i++) {
		...
		if (i > m)
		{
			...
			for (int j = 1; j <= value[i][0]; j++)
			{
				glTexCoord2f((poligono.getPontos()[(unsigned int)value[i][j]].getX() / 4), (poligono.getPontos()[(unsigned int) value[i][j]].getZ() / 4));
				glVertex3f(poligono.getPontos()[(unsigned int)value[i][j]].getX(), poligono.getPontos()[(unsigned int)value[i][j]].getY(), 
					poligono.getPontos()[(unsigned int)value[i][j]].getZ());
...
void lerArquivoConeCoqueiro(char* nomeArquivo)
{
	...
	for (i = 0; i < linhas + 1; i++) {
		...
		glutSolidCone(value[i][0], (GLint) value[i][1], (GLint) value[i][2], (GLint) value[i][3]);
...
void lerArquivoCilindroCoqueiro(char* nomeArquivo)
{
	...
	for (i = 0; i < linhas + 1; i++) {
		...
		gluCylinder(quad, (GLint) value[i][0], (GLint) value[i][1], (GLint) value[i][2], (GLint) value[i][3], (GLint) value[i][4]);
		glPopMatrix();
...
void lerArquivoEsferaCoqueiro(char* nomeArquivo)
{
	...
	for (i = 0; i < linhas + 1; i++) {
		...
		glTranslatef(6.0f, 10.0f, 1.5f);
		glutSolidSphere(value[i][0], (GLint) value[i][1], (GLint) value[i][2]);
		//gluCylinder(quad, 3.0, 3.0, 1.0, 10.0, 10.0);
		glPopMatrix();
...
void cenario() {
	...
	gluSphere(quad, 350.0f, (GLint) 10.0f, (GLint) 10.0f);
	glPopMatrix();
...
void moinho() {
	...
	gluQuadricNormals(quad, GLU_SMOOTH);
	gluQuadricTexture(quad, GL_TRUE);
	gluCylinder(quad, 2, 1, 9.0f, (GLint) 10.0f, (GLint) 10.0f);
	glPopMatrix();
...
void paquito() {
	...
	gluQuadricNormals(quad, GLU_SMOOTH);
	gluQuadricTexture(quad, GL_TRUE);
	gluCylinder(quad, 0.25f, 0.25f, 20.0f, (GLint) 10.0f, (GLint) 10.0f);
	glPopMatrix();
	glPushMatrix();
...
void passaro() {
	...
	gluQuadricNormals(quad, GLU_SMOOTH);
	gluQuadricTexture(quad, GL_TRUE);
	gluCylinder(quad, (GLint) 0.5, (GLint) 0.2, (GLint) 2.5, (GLint) 10.0, (GLint) 10.0);
	glPopMatrix();
...
void lerCasa3_telha2(char* nomeArquivo)
{
	...
	for (i = 1; i < linhas + 1; i++) {
		...
		if (i > m)
		{
			...
			for (int j = 1; j <= value[i][0]; j++)
			{
				glTexCoord2f((poligono.getPontos()[(unsigned int)value[i][j]].getX()) / 15, (poligono.getPontos()[(unsigned int)value[i][j]].getZ() / 15));
				glVertex3f(poligono.getPontos()[(unsigned int)value[i][j]].getX(), poligono.getPontos()[(unsigned int)value[i][j]].getY(), 
					poligono.getPontos()[(unsigned int)value[i][j]].getZ());
...
void Desenha() {
	...
	glPushMatrix();
	glTranslatef(-50.0f, 0.1f, 0.1f);
	glBindTexture(GL_TEXTURE_2D, texturas[AREIA]);
	glBegin(GL_QUADS);


	glNormal3f(0, 1, 0);
	glTexCoord2f(0.0f, 0.0f); glVertex3f(-50.0f, 1.0f, -200.0f);
	glTexCoord2f(0.0f, 80.0f); glVertex3f(-25.0f, 1.0f, -200.0f);
	glTexCoord2f(80.0f, 80.0f); glVertex3f(-25.0f, 1.0f, 0.0f);
	glTexCoord2f(80.0f, 0.0f); glVertex3f(-50.0f, 1.0f, 0.0f);
	glEnd();
...
void Timer(int value)
{
	...
	if (delay_abertura < DELAY_ABERTURA) {
		delay_abertura += FPS;
		glutTimerFunc((unsigned int) FPS, Timer, 0);
	}
